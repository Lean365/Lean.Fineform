...
namespace LeanFine.Lf_Accounting
{
    public partial class costing_fc_q1 : PageBase
    {
        ...
        protected void Grid1_RowDataBound(object sender, GridRowEventArgs e)
        {
            ...
            {
                // 4月增减
                int DiffQty04 = Convert.ToInt32(row["DiffQty'04"]);
                FineUIPro.BoundField cDiffQty04 = Grid1.FindColumn("DiffQty'04") as FineUIPro.BoundField;
                if (DiffQty04 > 0)
                {
                    e.CellAttributes[cDiffQty04.ColumnIndex]["data-color"] = "color1";
                }
                if (DiffQty04 == 0)
                {
                    e.CellAttributes[cDiffQty04.ColumnIndex]["data-color"] = "color2";
                }
                if (DiffQty04 < 0)
                {
                    e.CellAttributes[cDiffQty04.ColumnIndex]["data-color"] = "color3";
                }
                // 5月增减
                int DiffQty05 = Convert.ToInt32(row["DiffQty'05"]);
                FineUIPro.BoundField cDiffQty05 = Grid1.FindColumn("DiffQty'05") as FineUIPro.BoundField;
                if (DiffQty05 > 0)
                {
                    e.CellAttributes[cDiffQty05.ColumnIndex]["data-color"] = "color1";
                }
                if (DiffQty05 == 0)
                {
                    e.CellAttributes[cDiffQty05.ColumnIndex]["data-color"] = "color2";
                }
                if (DiffQty05 < 0)
                {
                    e.CellAttributes[cDiffQty05.ColumnIndex]["data-color"] = "color3";
                }

                // 6月增减
                int DiffQty06 = Convert.ToInt32(row["DiffQty'06"]);
                FineUIPro.BoundField cDiffQty06 = Grid1.FindColumn("DiffQty'06") as FineUIPro.BoundField;
                if (DiffQty06 > 0)
                {
                    e.CellAttributes[cDiffQty06.ColumnIndex]["data-color"] = "color1";
                }
                if (DiffQty06 == 0)
                {
                    e.CellAttributes[cDiffQty06.ColumnIndex]["data-color"] = "color2";
                }
                if (DiffQty06 < 0)
                {
                    e.CellAttributes[cDiffQty06.ColumnIndex]["data-color"] = "color3";
                }
            }
        }
        ...
        {            // 在操作之前进行权限检查
            ...
            if (q_ym.Any())
            {
                ...
                if (q.Count() != 0)
                {
                    ...
                    if (Grid1.RecordCount != 0)
                    {
                        ExportHelper.EpplusToExcel(result, Prefix_XlsxName, Export_FileName, "DTA Forcast明细");
                    }
                    else
