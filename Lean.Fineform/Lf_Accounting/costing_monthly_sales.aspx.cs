...
{
    public partial class costing_monthly_sales : PageBase
    {
        #region ViewPower
        ...
        private void BindGrid()
        {
            try
            {
                if (rbtnFirstAuto.Checked)
                {
                    ...
                    var q_BomCost = from a in DB.Fico_Monthly_Bom_Costs
                                    where a.Bc_YM.CompareTo(BomDate) == 0
                                    select a;

                    var q_SalesInv = from b in DB.Fico_Monthly_Sales
        ...
        {            // 在操作之前进行权限检查
            ...
            Prefix_XlsxName = "SalesCost_" + DpEndDate.SelectedDate.Value.ToString("yyyyMM");
            ...
            if (Grid1.RecordCount != 0)
            {
                ...
                ExportHelper.EpplusToExcel(ExportHelper.GetGridDataTable(Grid1), Prefix_XlsxName, Export_FileName, "DTA 销售成本明细");
