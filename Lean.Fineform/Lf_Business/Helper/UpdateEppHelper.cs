...
{
    public class UpdateEppHelper : PageBase
    {
        public static Decimal ophQty, orderQty;

        /// <summary>
        /// 制一课更新不良表（Pp_P1d_Epp_Defects）中实际生产数量，条件按日期，订单，班组
        /// </summary>
        /// <param name="strPorder"></param>
        /// <param name="strPdate"></param>
        /// <param name="strPline"></param>
        public static void P1d_Defect_Epp_Update_For_Realqty(string strPorder, string strPdate, string strPline, string uid)
        {
            int realQty = 0;
        ...
        /// </summary>
        /// <param name="strPorder"></param>
        public static void P1d_Defect_Epp_Orders_Update_For_Realqty(string strPorder, string uid, string strProdept)
        {
            int realQty = 0;
        ...
        /// </summary>
        /// <param name="strPorder"></param>
        public static void P1d_Defect_Epp_Orders_Update_For_NoBadQty(string strPorder, string uid, string strProdept)
        {
            int noQty = 0;
            int okQty = 0;
            var noqs =
                from p in DB.Pp_P1d_Epp_Defects
                where p.IsDeleted == 0
                //where p.Proorder == strPorder
                group p by new
                {
            ...
            //统计无不良台数（无不良录入时）
            var ids = new HashSet<string>(DB.Pp_P1d_Epp_Defects.Select(x => x.Prodate + x.Proorder + x.Prolinename));
            var results = DB.Pp_P1d_Epp_OutputSubs.Where(x => !ids.Contains(x.Prodate + x.Proorder + x.Prolinename) && x.Proorder == strPorder && x.IsDeleted == 0);
        ...
        public static void P1d_Defect_Epp_For_Check(string strPorder, string strPdate, string Pline)
        {
            int noQty = 0;
        ...
        /// </summary>
        /// <param name="strPorder"></param>
        public static void P1d_Defect_Epp_Orders_Update_For_BadQtyTotal(string strPorder, string uid, string strProdept)
        {
            int BadTotalQty = 0;
        ...
        {
            Pp_P1d_Epp_Output current = DB.Pp_P1d_Epp_Outputs

                .Where(u => u.GUID == strGuid).FirstOrDefault();

            String OphOrder = current.Proorder;

            //String SS = "SELECT[Proorder], SUM([Prorealqty])[Prorealqty]  FROM[OneHerba].[dbo].[Pp_P1d_Epp_OutputSubs] SUB" +
            //           "   LEFT JOIN[dbo].[Pp_P1d_Epp_Outputs] OPH ON SUB.OPHID = OPH.OPHID WHERE [Proorder]='"+ OphOrder + "' GROUP BY[Proorder]" +
            //           "   UNION ALL" +
            //           "   (SELECT Porderno, Porderqty FROM[dbo].[proOrders]" +
            //           "   WHERE Porderno = (" +
            //           "   SELECT[Proorder] FROM[dbo].[Pp_P1d_Epp_Outputs] WHERE [Proorder]='" + OphOrder + "'));";

            //语句描述：连接查询表。
            var q =
            from sub in DB.Pp_P1d_Epp_OutputSubs
            join oph in DB.Pp_P1d_Epp_Outputs on sub.GUID equals oph.GUID
            ...
            var qo =
                    from sub in DB.Pp_Orders
                        //join oph in DB.Pp_P1d_Epp_Outputs on sub.OPHID equals oph.OPHID
        ...
        public static void UpdateOrderRealQty(string OrderNo, string uid)//(Guid strGuid)
        {
            try
            {
                var q_count = (from a in DB.Pp_P1d_Epp_OutputSubs
                               where a.IsDeleted == 0
        ...
        public static void DelOutputAssySubs(int ID, string proLine, string proDate, string proLot, string proItem, string uid)
        {
            try
            {
                ...
                if (!q.Any())
                {
                    ...
                    // DB.Pp_P1d_Epp_OutputSubs.Where(l => l.Parent == ID).DeleteFromQuery();
                    DB.Pp_P1d_Epp_Outputs.Where(l => l.ID == ID).DeleteFromQuery();
                }
                //求和
                //var qs =
