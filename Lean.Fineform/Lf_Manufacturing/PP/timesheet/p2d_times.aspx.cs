...
{
    public partial class p2d_times : PageBase
    {
        #region ViewPower
        ...
        private void GridSummaryData(DataTable source)
        {
            ...
            {
                pTotal += Convert.ToDecimal(row["Prohandoffnum"]);
                rTotal += Convert.ToDecimal(row["Prohandofftime"]);
                ratio += Convert.ToDecimal(row["Prodowntime"]);
            }

            JObject summary = new JObject();
            //summary.Add("major", "全部合计");

            summary.Add("Prohandoffnum", pTotal.ToString("F0"));
            summary.Add("Prohandofftime", rTotal.ToString("F0"));
            summary.Add("Prodowntime", ratio.ToString("F0"));
            ...
            if (q.Any())
            {
                var qs = from g in q
                         ...
                         select new
                         {
                             ...
                             说明 = g.Probadmemo,
                             次数 = g.numProstopcou,
                             比例 = (g.numProstopcou / Allcount) * 100 / 100,
                         };
