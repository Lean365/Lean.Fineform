...
namespace LeanFine.Lf_Manufacturing.PP.poor
{
    public partial class p2d_defect_edit : PageBase
    {
            ...
            else
            {
                if (newAddedList.Count != 0)
                {
                    for (int i = newAddedList.Count - 1; i >= 0; i--)
                    {
                        ...
                        //无不良台数
                        item.Prodzeroefects = int.Parse(Prodzeroefects);
        ...
        private void InsertDefectDataRow(Dictionary<string, object> rowDict, DataRow rowData)
        {
            ...
            // 对策
            UpdateDataRow("Prodefectlocation", rowDict, rowData);
            // 对策
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            ...
            //更新不具合合计
            UpdatingP2dHelper.Pp_Defect_P2d_BadTotal_Update(this.lblProdate.Text, lblProlinename.Text, strPorder, GetIdentityName());
            //更新无不良台数
        ...
        private void UpdateNobadqty()
        {
            ...
            {
                string ddate = this.lblProdate.Text;
                var Pp_Def = DB.Pp_P2d_Defects.Where(u => u.IsDeleted == 0 && u.Prolinename.Contains(lblProlinename.Text) && u.Prolot.Contains(lblProlot.Text) && u.Prodate.Contains(ddate));
                foreach (var Pp_P2d_Defects in Pp_Def)
                {
                    Pp_P2d_Defects.Prodzeroefects = int.Parse(this.numProdzeroefects.Text);
