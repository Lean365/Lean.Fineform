...
{
    public partial class p2d_inspection_defect_monthly : PageBase
    {
        #region ViewPower
        ...
        private void BindGrid()
        {
            try
            {
                ...
                {
                    q = q.Where(u => u.Promodel.Contains(searchText) || u.Propcbtype.Contains(searchText) || u.Prodshiftname.Contains(searchText) || u.Proorder.Contains(searchText) || u.Prolinename.Contains(searchText) || u.Probadtype.Contains(searchText)); //|| u.CreateDate.Contains(searchText));
                }
                //q = q.Distinct();
                //else
                //{
        ...
        protected void BtnExport_Click(object sender, EventArgs e)
        {
            ...
            Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + "_P2d_Inspection_Report";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(edate))
                {
                    q = q.Where(u => u.Proinspdate.Substring(0, 6).CompareTo(edate) == 0);
                }
                ...
                if (q.Any())
                {
                    var qs = q.Select(E => new
                    {
                        ...
                        机种名称 = E.Promodel,
                        板别 = E.Propcbtype,
                        目视线别 = E.Provisualtype,
                        AOI线别 = E.Provctype,
                        B面实装 = E.Prosideadate,
                        T面实装 = E.Prosidebdate,
                        生产班别 = E.Prodshiftname,
                        检查员 = E.Procensor,
                        工单号 = E.Proorder,
                        批次 = E.Prolot,
                        LOT台数 = E.Proorderqty,
                    ...
                    ExportHelper.Inspection_XlsxFile(ConvertHelper.LinqConvertToDataTable(qs), "Pi" + DpEndDate.SelectedDate.Value.ToString("yyyyMM"), Export_FileName, DpEndDate.SelectedDate.Value.ToString("yyyyMM"));
                }
                else
