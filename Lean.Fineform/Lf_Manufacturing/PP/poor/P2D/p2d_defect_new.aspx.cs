...
{
    public partial class p2d_defect_new : PageBase
    {
        #region ViewPower
        ...
        public static string Prolot, Prolinename, Prodate, Prorealqty, Prodefectsymptom, Proorder, Prodefectcause, Prodzeroefects, Proorderqty, Promodel, Promodelqty, Probadqty, Probadtotal, Probadamount, Prodefectcategory, Prodefectlocation;
        ...
        private void BindGrid()
        {
            ...
            q = q.Where(u => u.IsDeleted == 0 && u.Prolinename.Contains(ddlProlinename.SelectedItem.Text) && u.Proorder.Contains(ddlProorder.SelectedItem.Text) && u.Prodate.Contains(ddate));
        ...
        private void BindDdlproOrder()
        {
            ...
            ddlProorder.DataTextField = "Prolot";
            ddlProorder.DataValueField = "Prolot";
        ...
        protected void prolinename_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                numProrealqty.Text = "0";
                lblProdzeroefects.Text = "0";
                lblPromodel.Text = "";
                //proorder.Text = "";
        ...
        {
            if (ddlProorder.SelectedIndex != -1 && ddlProorder.SelectedIndex != 0 && ddlProlinename.SelectedIndex != -1 && ddlProlinename.SelectedIndex != 0)
            {
                this.lblProlot.Text = ddlProorder.SelectedItem.Text;
                string sorder = ddlProorder.SelectedItem.Text.Substring(0, ddlProorder.SelectedItem.Text.IndexOf(","));
                ...
                if (qs.Any())
                {
                    ...
                    this.lblProorderqty.Text = ddlProorder.SelectedItem.Text.Substring(ddlProorder.SelectedItem.Text.IndexOf(",") + 1, ddlProorder.SelectedItem.Text.Length - ddlProorder.SelectedItem.Text.IndexOf(",") - 1);
                ...
                if (newAddedList.Count != 0)
                {
                    for (int i = 0; i < newAddedList.Count; i++)
                    {
                        ...
                        foreach (DataColumn col in tb.Columns)
                        {
                            ...
                            //无不良台数
                            Prodzeroefects = tb.Rows[tb.Rows.Count - 1]["Prodzeroefects"].ToString();
                            //生产实绩
                            Prorealqty = tb.Rows[tb.Rows.Count - 1]["Prorealqty"].ToString();
                            //不良症状
                            Prodefectsymptom = tb.Rows[tb.Rows.Count - 1]["Prodefectsymptom"].ToString();
                            //不良个所
                            Prodefectlocation = tb.Rows[tb.Rows.Count - 1]["Prodefectlocation"].ToString();
                            //不良原因
                            Prodefectcause = tb.Rows[tb.Rows.Count - 1]["Prodefectcause"].ToString();
                            //不良件数
                            Probadqty = tb.Rows[tb.Rows.Count - 1]["Probadqty"].ToString();
                            //不良件数
                            Prodefectcategory = tb.Rows[tb.Rows.Count - 1]["Prodefectcategory"].ToString();
                        }
                        ...
                        //订单
                        item.Proorder = ddlProorder.Text;
                        //班组
                        item.Prolinename = Prolinename;
                        //日期
                        ...
                        //无不良台数
                        item.Prodzeroefects = int.Parse(lblProdzeroefects.Text);
                        ...
                        //不良个所
                        item.Prodefectlocation = Prodefectlocation;
                        //不良原因
                ...
                if (newAddedList.Count != 0)
                {
                    for (int i = newAddedList.Count - 1; i >= 0; i--)
                    {
                        ...
                        foreach (DataColumn col in tb.Columns)
                        {
                            ...
                            //无不良台数
                            Prodzeroefects = tb.Rows[0]["Prodzeroefects"].ToString();
                            //生产实绩
                            Prorealqty = tb.Rows[0]["Prorealqty"].ToString();
                            //不良症状
                            Prodefectsymptom = tb.Rows[0]["Prodefectsymptom"].ToString();

                            //不良个所
                            Prodefectlocation = tb.Rows[0]["Prodefectlocation"].ToString();
                            //不良原因
                            Prodefectcause = tb.Rows[0]["Prodefectcause"].ToString();
                            //不良件数
                            Probadqty = tb.Rows[0]["Probadqty"].ToString();
                            //不良件数
                            Prodefectcategory = tb.Rows[0]["Prodefectcategory"].ToString();
                        }
                        Pp_P2d_Defect item = new Pp_P2d_Defect();
        ...
        private DataRow CreateNewData(DataTable table, Dictionary<string, object> newAddedData)
        {
            ...
            //订单
            rowData["Proorder"] = ddlProorder.SelectedItem.Text;
            //班组
            rowData["Prolinename"] = ddlProlinename.SelectedItem.Text;
            //生产日期
            rowData["Prodate"] = dpProdate.SelectedDate.Value.ToString("yyyyMMdd");
            //订单台数
            rowData["Proorderqty"] = lblProorderqty.Text;
            //机种台数
            //rowData["Promodelqty"] = proorder.Text;
            //生产台数
            rowData["Prorealqty"] = this.numProrealqty.Text;
            //无不良台数
            rowData["Prodzeroefects"] = lblProdzeroefects.Text;
            //区分

            rowData["Prodefectcategory"] = Prodefectcategory;
        ...
        {
            // 区分
            UpdateDataRow("Prodefectcategory", rowDict, rowData);
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            try
            {
                if (Decimal.Parse(lblProdzeroefects.Text) > Decimal.Parse(numProrealqty.Text))
                {
            ...
            string lot = ddlProorder.SelectedItem.Text.Substring(0, ddlProorder.SelectedItem.Text.IndexOf(","));
            ...
            //更新不具合合计
            UpdatingP2dHelper.Pp_Defect_P2d_BadTotal_Update(this.dpProdate.SelectedDate.Value.ToString("yyyyMMdd"), ddlProlinename.SelectedItem.Text, ddlProorder.SelectedItem.Text.Substring(ddlProorder.SelectedItem.Text.IndexOf(",") + 1, ddlProorder.SelectedItem.Text.Length - ddlProorder.SelectedItem.Text.IndexOf(",") - 1), GetIdentityName());
            //更新无不良台数
            //Common.UpdatenobadAmount(this.DefDate.SelectedDate.Value.ToString("yyyyMMdd"), prolinename.SelectedItem.Text, prolot.SelectedItem.Text.Substring(prolot.SelectedItem.Text.IndexOf(",") + 1, prolot.SelectedItem.Text.Length - prolot.SelectedItem.Text.IndexOf(",") - 1), int.Parse(Prodzeroefects.Text));
