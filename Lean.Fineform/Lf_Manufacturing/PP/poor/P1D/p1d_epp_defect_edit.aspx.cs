...
{
    public partial class p1d_epp_defect_edit : PageBase
    {
        #region ViewPower
        ...
        private void LoadData()
        {
            ...
            //IQueryable<Pp_P1d_Epp_Defect> q = DB.Pp_P1d_Epp_Defects;
            //DefDatatable = CopyToDataTable(q);

            //获取SQL数据表
            DefDatatable = ConvertHelper.GetDataTable("SELECT * FROM Pp_P1d_Epp_Defect");
        ...
        private void BindData()
        {
            ...
            Pp_P1d_Epp_Defect current = DB.Pp_P1d_Epp_Defects.Find(id);
            ...
            statProeppnotes = current.Proeppnotes.ToString();

            if (!String.IsNullOrEmpty(current.Prodzeroefects.ToString()))
            {
                numProdzeroefects.Text = current.Prodzeroefects.ToString();
            }
            else
            {
                numProdzeroefects.Text = current.Prorealqty.ToString();
            }
            //更新前无不良台数
            Oldnobadqty = int.Parse(numProdzeroefects.Text);
            ...
            string BeforeOperateNotes = "Edit改修不良* " + BeforeEdit + " *Edit改修不良 的记录可能将被修改";
            OperateLogHelper.InsNetOperateNotes(userid, BeforeOperateType, "改修管理", "改修修改", BeforeOperateNotes);
        }
        ...
        private void BindGrid()
        {
            ...
            // 排列和数据库分页
            q = SortAndPage<Pp_P1d_Epp_Defect>(q, Grid1);
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete")
            {
                ...
                string OperateNotes = "Del改修不良* " + Contectext + " *Del改修不良 的记录可能将被删除";
                OperateLogHelper.InsNetOperateNotes(GetIdentityName(), OperateType, "改修管理", "改修删除", OperateNotes);

                //删除记录
                //DB.Pp_P1d_Epp_Defects.Where(l => l.ID == del_ID).Delete();
        ...
        private void EditDefectDataRow()
        {
            ...
            foreach (int rowIndex in deletedRows)
            {
                ...
                Pp_P1d_Epp_Defect item = DB.Pp_P1d_Epp_Defects
            ...
            if (AppendToEnd)
            {
                if (newAddedList.Count != 0)
                {
                    for (int i = 0; i < newAddedList.Count; i++)
                    {
                        ...
                        Pp_P1d_Epp_Defect item = new Pp_P1d_Epp_Defect();
                        ...
                        item.Proeppnotes = statProeppnotes;
                        item.CreateDate = DateTime.Now;
                        item.Creator = GetIdentityName();
                        DB.Pp_P1d_Epp_Defects.Add(item);
                        DB.SaveChanges();
                        ...
                        string OperateNotes = "New改修不良*" + Contectext + "*New改修不良 的记录已经将新增";
                        OperateLogHelper.InsNetOperateNotes(GetIdentityName(), OperateType, "改修管理", "改修新增", OperateNotes);
                    }
                }
                //else
                //{
                ...
                //    //种类
                //    Pp_P1d_Epp_Defectcode cclass = DB.Pp_P1d_Epp_Defectcodes
                ...
                //    //代码
                //    Pp_P1d_Epp_Defectcode ccode = DB.Pp_P1d_Epp_Defectcodes
        ...
        private DataRow CreateNewData(DataTable table, Dictionary<string, object> newAddedData)
        {
            ...
            //班组
            rowData["Prolinename"] = lblProlinename.Text;
            //生产日期
            rowData["Prodate"] = lblProdate.Text;
            //订单台数
            rowData["Proorderqty"] = (int)Math.Round(decimal.Parse(lblProorderqty.Text));
            //机种台数
            ...
            rowData["IsDeleted"] = 0;
            rowData["Proeppnotes"] = statProeppnotes;
        ...
        private static void UpdateDefectDataRow(Dictionary<string, object> rowDict, DataRow rowData)
        {
            ...
            string BeforeOperateType = "修改";
            string BeforeOperateNotes = "beEdit改修不良* " + BeforeContectext + " *beEdit改修不良 的记录可能将被修改";
            OperateLogHelper.InsNetOperateNotes(userid, BeforeOperateType, "改修管理", "改修修改", BeforeOperateNotes);
            ...
            Pp_P1d_Epp_Defect edititem = DB.Pp_P1d_Epp_Defects
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            ...
            string pdate = "";
            string strPlot = "";
            nobadqty = int.Parse(numProdzeroefects.Text);
            strPorder = lblProorder.Text;
            pdate = this.lblProdate.Text.Substring(0, 6);

            strPlot = lblProlot.Text;

            //更新生产实绩Pp_P1d_Epp_Defects
            UpdateEppHelper.P1d_Defect_Epp_Update_For_Realqty(strPorder, this.lblProdate.Text, lblProlinename.Text, GetIdentityName());
            //更新生产实绩Pp_Defect_P1d_Orders
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_Realqty(strPorder, userid, "ASSYE");

            //更新无不良台数Pp_Defect_P1d_Orders
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_NoBadQty(strPorder, userid, "ASSYE");

            //更新不具合合计
            UpdateEppHelper.UpdatebadAmount(this.lblProdate.Text, lblProlinename.Text, strPlot, GetIdentityName(), "ASSYE");
        ...
        {
            //判断无不良台数是否变化
            if (Oldnobadqty != int.Parse(numProdzeroefects.Text))
            {
                string ddate = this.lblProdate.Text;
                var Pp_Def = DB.Pp_P1d_Epp_Defects.Where(u => u.IsDeleted == 0 && u.Prolinename.Contains(lblProlinename.Text) && u.Prolot.Contains(lblProlot.Text) && u.Prodate.Contains(ddate));
                foreach (var Pp_P1d_Epp_Defects in Pp_Def)
                {
                    Pp_P1d_Epp_Defects.Prodzeroefects = int.Parse(this.numProdzeroefects.Text);

                    Pp_P1d_Epp_Defects.Modifier = GetIdentityName();
                    Pp_P1d_Epp_Defects.ModifyDate = DateTime.Now;
                }
                DB.SaveChanges();
            }
        }
