...
namespace LeanFine.Lf_Manufacturing.PP.poor
{
    public partial class p1d_epp_defect_new : PageBase
    {
...

        {
            if (dpProdate.SelectedDate.HasValue)
            {
                ...
                var q = from a in DB.Pp_P1d_Epp_Outputs
                        ...
                        where a.Prodate.Contains(statProdate) && !(from d in DB.Pp_P1d_Epp_Defects
                                                                   where d.IsDeleted == 0
                                                                   where d.Prodate == statProdate
        ...
        private DataRow CreateNewData(DataTable table, Dictionary<string, object> newAddedData)
        {
            ...
            //生产台数
            rowData["Prorealqty"] = (int)Math.Round(decimal.Parse(this.lblProrealqty.Text));
            //无不良台数
            rowData["Prodzeroefects"] = int.Parse(numProdzeroefects.Text);
            //区分
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            ...
            UpdateEppHelper.P1d_Defect_Epp_Update_For_Realqty(order, this.dpProdate.SelectedDate.Value.ToString("yyyyMMdd"), ddlProlinename.SelectedItem.Text, GetIdentityName());
            ...
            //更新不具合合计
            UpdateEppHelper.UpdatebadAmount(this.dpProdate.SelectedDate.Value.ToString("yyyyMMdd"), ddlProlinename.SelectedItem.Text, ddlProlot.SelectedItem.Text.Substring(ddlProlot.SelectedItem.Text.IndexOf(",") + 1, ddlProlot.SelectedItem.Text.Length - ddlProlot.SelectedItem.Text.IndexOf(",") - 1), GetIdentityName(), "ASSYE");
