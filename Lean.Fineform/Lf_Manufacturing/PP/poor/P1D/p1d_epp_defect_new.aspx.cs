...
{
    public partial class p1d_epp_defect_new : PageBase
    {
        #region ViewPower
        ...
        public static string ProNote, Prolot, Prolinename, Prodate, Prorealqty, Probadnote, Proorder, Probadreason, Pronobadqty, Proorderqty, Promodel, Promodelqty, Probadqty, Probadtotal, Probadamount, Prongdept, Probadset;
        ...
        private void BindDdlprolot()
        {
            ...
            if (this.DefDate.SelectedDate.Value.ToString("yyyyMMdd").Length == 8)
            {
                ...
                var q = from p in DB.Pp_P1d_Epp_Outputs
                        where p.Prodate.Contains(Prodate) && p.Prolinename.Contains(pline)
                            && !(from d in DB.Pp_P1d_Epp_Defects
                                 where d.IsDeleted == 0
...

        {
            if (DefDate.SelectedDate.HasValue)
            {
                ...
                //查询LINQ去重复
                var q = from a in DB.Pp_P1d_Epp_Outputs
                            //join b in DB.proEcnSubs on a.Porderhbn equals b.Proecnbomitem
                            //where b.Proecnno == strecn
                        where a.Prodate.Contains(Prodate) && !(from d in DB.Pp_P1d_Epp_Defects
                                                               where d.IsDeleted == 0
                                                               where d.Prodate == Prodate
        ...
        protected void prolot_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (prolot.SelectedIndex != -1 && prolot.SelectedIndex != 0 && prolinename.SelectedIndex != -1 && prolinename.SelectedIndex != 0)
            {
                ...
                var q_output = from a in DB.Pp_P1d_Epp_OutputSubs
                                   //join b in DB.Pp_P1d_Epp_OutputSubs on a.ID equals b.Parent.ID
                ...
                if (qs.Any())
                {
                    ...
                    proorderqty.Text = qs[0].Proorderqty.ToString();
                    promodelqty.Text = qs[0].Proorderqty.ToString();
                    ProNote = "Epp";
                    ...
                    //}
                    //机种总数量不再需要
                    //var modelqty = (from a in DB.Pp_P1d_Epp_OutputSubs
                ...
                //            " FROM [dbo].[Pp_P1d_Epp_OutputSubs] where Prodate='" + this.DefDate.SelectedDate.Value.ToString("yyyyMMdd") + "' and  [Prolot] = '" + prolot.SelectedItem.Text + "' and  [Prolinename]='" + prolinename.SelectedItem.Text + "'" +
        ...
        private void EditDefectDataRow()
        {
            ...
            //mysql = "select * from [dbo].[Pp_P1d_Epp_Defects];";
        ...
        private DataRow CreateNewData(DataTable table, Dictionary<string, object> newAddedData)
        {
            ...
            int s = (int)Math.Round(decimal.Parse(proorderqty.Text));
            //订单台数
            rowData["Proorderqty"] = s;//(int)Math.Round(3.5); // 结果为 4
            //机种台数
            //rowData["Promodelqty"] = proorder.Text;
            //生产台数
            rowData["Prorealqty"] = (int)Math.Round(decimal.Parse(this.prorealqty.Text));
            //无不良台数
            rowData["Pronobadqty"] = int.Parse(pronobadqty.Text);
            //区分
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            ...
            UpdateEppHelper.P1d_Defect_Epp_Update_For_Realqty(order, this.DefDate.SelectedDate.Value.ToString("yyyyMMdd"), prolinename.SelectedItem.Text, GetIdentityName());
            //更新生产实绩Pp_Defect_P1d_Orders
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_Realqty(order, userid, "ASSYE");

            //更新无不良台数Pp_Defect_P1d_Orders
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_NoBadQty(order, userid, "ASSYE");

            //更新不具合合计
            UpdateEppHelper.UpdatebadAmount(this.DefDate.SelectedDate.Value.ToString("yyyyMMdd"), prolinename.SelectedItem.Text, prolot.SelectedItem.Text.Substring(prolot.SelectedItem.Text.IndexOf(",") + 1, prolot.SelectedItem.Text.Length - prolot.SelectedItem.Text.IndexOf(",") - 1), GetIdentityName(), "ASSYE");
