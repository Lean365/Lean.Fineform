...
{
    public partial class p1d_defect_moc_finished : PageBase
    {
        #region ViewPower
        ...
        private void GridtoExcel()
        {
            ...
            {
                Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + FsearchText + "_" + "MocDefectReport";
            }
            else

            {
                Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + "_MocDefectReport";
            }
            // mysql = "SELECT [Prodate] 日付,[Prohbn] 品目,[Prost] ST,[Proplanqty] 計画台数,[Proworktime] 投入工数,[Proworkqty] 実績台数,[Prodirect] 直接人数,[Proworkst] 実績ST,[Prodiffst] ST差異,[Prodiffqty] 台数差異,[Proactivratio] 稼働率  FROM [dbo].[proOutputlinedatas] where left(Prodate,6)='" + DDLdate.SelectedText + "'";
            ...
            if (q.Any())
            {
                for (int i = 0; i < ma.Rows.Count; i++)
                {
                    ...
                    var q2 =
                            ...
                            {
                                p.Prodefectcategory,
                                Prodefectsymptom = (string.IsNullOrEmpty(p.Prorandomcard) ? "" : "(No:" + p.Prorandomcard + ")") + (p.Prodefectsymptom ?? ""),
                                p.Prodefectlocation,
                                p.Prodefectcause,
                                p.Probadqty,
                            };
        ...
        protected void BtnExport_Click(object sender, EventArgs e)
        {
            ...
            // mysql = "SELECT [Prodate] 日付,[Prohbn] 品目,[Prost] ST,[Proplanqty] 計画台数,[Proworktime] 投入工数,[Proworkqty] 実績台数,[Prodirect] 直接人数,[Proworkst] 実績ST,[Prodiffst] ST差異,[Prodiffqty] 台数差異,[Proactivratio] 稼働率  FROM [dbo].[proOutputlinedatas] where left(Prodate,6)='" + DDLdate.SelectedText + "'";
            Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + "_MocDefectDetails";
            //mysql = "EXEC DTA.dbo.SP_BOM_EXPAND '" + Prefix_XlsxName + "'";
            Export_FileName = Prefix_XlsxName + ".xlsx";
