...
{
    public partial class p1d_epp_defect_lot_finished : PageBase
    {
        #region ViewPower
        ...
        private void BindGrid()
        {
            try
            {
                ...
                var q = from a in DB.Pp_Defect_P1d_Orders
                        ...
                        join b in DB.Pp_P1d_Epp_Defects on a.Prolot equals b.Prolot
                        where a.IsDeleted == 0
                        where a.Prodept == ("ASSYE")
                        where b.IsDeleted == 0
        ...
        private void GridtoExcel()
        {
            ...
            {
                Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + FsearchText + "_" + "EppModifyDefectReport";
            }
            else

            {
                Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + "_LotEppModifyDefectReport";
            }
            // mysql = "SELECT [Prodate] 日付,[Prohbn] 品目,[Prost] ST,[Proplanqty] 計画台数,[Proworktime] 投入工数,[Proworkqty] 実績台数,[Prodirect] 直接人数,[Proworkst] 実績ST,[Prodiffst] ST差異,[Prodiffqty] 台数差異,[Proactivratio] 稼働率  FROM [dbo].[proOutputlinedatas] where left(Prodate,6)='" + DDLdate.SelectedText + "'";
        ...
        protected void BtnExport_Click(object sender, EventArgs e)
        {
            ...
            //在库明细查询SQL
            string Prefix_XlsxName, Export_FileName, SheetName;
            SheetName = "D" + DpEndDate.SelectedDate.Value.ToString("yyyyMM");
            // mysql = "SELECT [Prodate] 日付,[Prohbn] 品目,[Prost] ST,[Proplanqty] 計画台数,[Proworktime] 投入工数,[Proworkqty] 実績台数,[Prodirect] 直接人数,[Proworkst] 実績ST,[Prodiffst] ST差異,[Prodiffqty] 台数差異,[Proactivratio] 稼働率  FROM [dbo].[proOutputlinedatas] where left(Prodate,6)='" + DDLdate.SelectedText + "'";
            Prefix_XlsxName = DpEndDate.SelectedDate.Value.ToString("yyyyMM") + "_EppModifyDefectDetails";
            //mysql = "EXEC DTA.dbo.SP_BOM_EXPAND '" + Prefix_XlsxName + "'";
            Export_FileName = Prefix_XlsxName + ".xlsx";
