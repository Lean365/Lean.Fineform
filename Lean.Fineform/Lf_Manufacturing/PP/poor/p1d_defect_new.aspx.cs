    ...
    public partial class p1d_defect_new : PageBase
    {
...

        {
            ...
            if (DefDate.SelectedDate.HasValue)
            {
                ...
                var q = from a in DB.Pp_Outputs
                        ...
                        where a.Prodate.Contains(Prodate) && !(from d in DB.Pp_Defect_P1ds
                                                               where d.isDelete == 0
                                                               where d.Prodate == Prodate
        ...
        protected void prolot_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (prolot.SelectedIndex != -1 && prolot.SelectedIndex != 0 && prolinename.SelectedIndex != -1 && prolinename.SelectedIndex != 0)
            {
                ...
                var q_output = from a in DB.Pp_OutputSubs
                                   //join b in DB.Pp_OutputSubs on a.ID equals b.Parent.ID
                ...
                if (qs.Any())
                {
                    ...
                    //机种总数量不再需要
                    //var modelqty = (from a in DB.Pp_Outputsubs
                ...
                //            " FROM [dbo].[Pp_Outputsubs] where Prodate='" + this.DefDate.SelectedDate.Value.ToString("yyyyMMdd") + "' and  [Prolot] = '" + prolot.SelectedItem.Text + "' and  [Prolinename]='" + prolinename.SelectedItem.Text + "'" +
        ...
        private void EditDefectDataRow()
        {
            ...
            //mysql = "select * from [dbo].[Pp_Defect_P1ds];";
