...
namespace Lean.Fineform.Lf_Manufacturing.PP.daily
{
    public partial class p2d_output_line_qty : PageBase
    {
        ...
        private void BindGrid()
        {
            ...
            "from(select distinct Prolinename from Pp_OutputSub) as a " +
            "select @sql = @sql + ' from Pp_OutputSub group by left(Prodate,6) order by left(Prodate,6)' " +
            "exec(@sql)";
                ...
                string LotSql = "declare @sql varchar(8000) "+
                            ...
                            "select @sql = @sql + ',isnull (sum(case Prolinename when ''' + Prolinename + ''' then Prorealqty end),0) as [' + Prolinename + ']' "+
                            "from(select distinct Prolinename from Pp_OutputSub) as a "+
                            "select @sql = @sql + ' from Pp_OutputSub group by left(Prodate,6),Prolot order by left(Prodate,6),Prolot' "+
                            "exec(@sql)";
           DataTable dtLot= ConvertHelper.GetDataTable(LotSql);
                ...
                {
                    var q_all = from p in DB.Pp_OutputSubs
                ...
                {
                    var q_normal = from p in DB.Pp_OutputSubs
                                       //join b in DB.Pp_Outputs on p.Parent.ID equals b.ID
                                   where p.isDelete == 0
                                   where p.Prorealtime != 0 || p.Prolinestopmin != 0
                ...
                {
                    var q_rework = from p in DB.Pp_OutputSubs
                                       //join b in DB.Pp_Outputs on p.Parent.ID equals b.ID
                                   where p.isDelete == 0
                                   where p.Prorealtime != 0 || p.Prolinestopmin != 0
                                   where p.Proorder.Substring(0, 2).CompareTo("44") != 0
