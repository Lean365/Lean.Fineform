...
{
    public partial class p1d_epp_daily_date_new : PageBase
    {
        ...
        private void LoadData()
        {
            ...
            if (compNum == -1)
            {
                this.pikProStartdate.SelectedDate = DateTime.Now.AddDays(-1);
                this.pikProStartdate.MinDate = dt_First;
                this.pikProStartdate.MaxDate = dt_Final;
            }
            if (compNum == 1)
            {
                this.pikProStartdate.SelectedDate = DateTime.Now.AddDays(-1);
                this.pikProStartdate.MinDate = lastFirst;
                this.pikProStartdate.MaxDate = dt_Final;
            }
        ...
        //    //    Pp_Output current = DB.Pp_P1d_Epp_Date_Outputs.Find(del_ID);
        //    //    //删除日志
        ...
        {
            Pp_P1d_Epp_Date_Output item = new Pp_P1d_Epp_Date_Output();

            //ophID();
            //item.OPHID = int.Parse(OPHID) + 1 + 1000;
            item.GUID = Guid.NewGuid();
            //item.Prolineclass = ddlProlinename.SelectedValue.ToString();
            item.Proordertype = ProOrderType;
            item.Prolinename = ddlProlinename.SelectedItem.Text;

            item.ProStartdate = pikProStartdate.SelectedDate.Value.ToString("yyyyMMdd");
            item.ProEnddate = pikProEnddate.SelectedDate.Value.ToString("yyyyMMdd");
            ...
            item.Prost = Decimal.Parse(numProst.Text);
            //item.Prosubst = Decimal.Parse(prosubst.Text);
            item.Prostdcapacity = Decimal.Parse(lblProlotqty.Text);
            ...
            DB.Pp_P1d_Epp_Date_Outputs.Add(item);
            DB.SaveChanges();
            //读取ID值
            ...
            string Contectext = ParentID + "," + ddlProlinename.SelectedItem.Text + "," + pikProStartdate.SelectedDate.Value.ToString("yyyyMMdd") + "," + lblProlot.Text + "," + lblProhbn.Text + "," + lblProstdcapacity.Text;
            ...
            SaveSubItem();
            //删除无单身数据
            UpdateP1dHelper.DelOutputAssySubs(ParentID, ddlProlinename.SelectedItem.Text, pikProStartdate.SelectedDate.Value.ToString("yyyyMMdd"), lblProlot.Text, lblProhbn.Text, GetIdentityName());
        }
        ...
        private void SaveSubItem()
        {
            try
            {
                Pp_P1d_Epp_Date_OutputSub item = new Pp_P1d_Epp_Date_OutputSub();
                ...
                //时间
                item.Prostime = pikProStartdate.SelectedDate.Value.ToString("yyyyMMdd");
                item.Proetime = pikProEnddate.SelectedDate.Value.ToString("yyyyMMdd");
                //item.Udf001 = prodate.SelectedDate.Value.ToString("yyyyMMdd");
                //item.Udf002 = this.ddlProlinename.SelectedItem.Text;
                item.Remark = txtRemark.Text;
                item.CreateDate = DateTime.Now;
                item.Creator = GetIdentityName();
                item.IsDeleted = 0;
                DB.Pp_P1d_Epp_Date_OutputSubs.Add(item);
                DB.SaveChanges();

                //新增日志

                string Newtext = ParentID + "," + pikProStartdate.SelectedDate.Value.ToString("yyyyMMdd") + "," + ddlProlinename.SelectedItem.Text + "," + pikProEnddate.SelectedDate.Value.ToString("yyyyMMdd") + "," + lblProlot.Text + "," + lblProhbn.Text + "," + lblProstdcapacity.Text;
        ...
        private void SaveOrderDefect()
        {
            try
            {
                ...
                item.Prolot = lblProlot.Text;
                item.Prolinename = ddlProlinename.SelectedItem.Text;
                item.Prodate = pikProEnddate.SelectedDate.Value.ToString("yyyyMMdd");
                item.Proorder = ddlProorder.SelectedItem.Text;
                ...
                //新增日志
                string Contectext = lblProlot.Text + ddlProlinename.SelectedItem.Text + "," + pikProEnddate.SelectedDate.Value.ToString("yyyyMMdd") + "," + lblProlot.Text + "," + ddlProorder.SelectedItem.Text + "," + lblProlotqty.Text;
                string OperateType = "新增";
        ...
        private void SaveOrderDefectUpdate()
        {
            ...
            var line = (from p in DB.Pp_P1d_Epp_Date_OutputSubs
                            //join b in DB.Pp_P1d_Epp_Date_Outputs on p.OPHID equals b.OPHID
        ...
        private void Distinctdata()
        {
            ...
            Pp_Defect_P1d_Order currentOrder = DB.Pp_Defect_P1d_Orders.Where(u => u.Proorder == inputOrder && u.Prodept.Contains("ASSYE")).FirstOrDefault();
            ...
            Pp_Defect_P1d_Lot currentLot = DB.Pp_Defect_P1d_Lots.Where(u => u.Prolot == inputLot && u.Prodept.Contains("ASSYE")).FirstOrDefault();
                ...
                try
                {
                    ...
                    if (q.Any())
                    {
                        ...
                        if (qs[0].Prost != 0)
                        {
                            ...
                            numProst.Text = qs[0].Prost.ToString();
                    ...
                    {
                        // 参数错误，首先弹出Alert对话框然后关闭弹出窗口
                        Alert.ShowInTop("此物料没有机种或ST，请通知电脑或技术部门录入！", String.Empty, ActiveWindow.GetHideReference());
                        //return;
                    }
                }
                catch (ArgumentNullException Message)
        ...
        protected void numProst_TextChanged(object sender, EventArgs e)
        {
            HourQty();
        }
        //计算标准产能
        ...
        {
            decimal stdiff = (decimal.Parse(numProst.Text));
            if (stdiff != 0)
            {
                OPHRate();
                decimal rate = (decimal)Prorate / 100;
                lblProstdcapacity.Text = ((decimal.Parse(numProdirect.Text) * 60) / (decimal.Parse(numProst.Text)) * rate).ToString("0.00");
            }
        }
    }
}