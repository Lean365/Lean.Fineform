...
{
    public partial class p1d_epp_daily_sub_edit : PageBase
    {
        #region ViewPower
        ...
        private void LoadData()
        {
            ...
            BindDdlStop();
            Rate();
            MemoText.Text = String.Format("<div style=\"margin-bottom:10px;color: #0000FF;\"><strong>填写说明：</strong></div><div>1.生产工数每1小时，参与OPH计划台数统计。</div><div>2.同一时段生产多个工单时，计划台数系统自动处理，作业工数计算方法：<strong>多个工单具体的实绩工数+停线时间</strong></div><div>3.实绩工数的计算公式：<strong>作业工数-停线时间</strong></div><div>4.实绩工数不能为负数</div><div>4.生产实绩不能超过工单数量</div>");
        ...
        private void BindGrid()
        {
            ...
            IQueryable<Pp_P1d_Epp_OutputSub> q = DB.Pp_P1d_Epp_OutputSubs; //.Include(u => u.Dept);
        ...
        {
            Pp_P1d_Epp_OutputSub item = DB.Pp_P1d_Epp_OutputSubs

                .Where(u => u.ID == rowID).FirstOrDefault();

            Pp_P1d_Epp_Output main_item = DB.Pp_P1d_Epp_Outputs
            ...
            //同一小时生产多个工单
            var pcount = from a in DB.Pp_P1d_Epp_OutputSubs
            ...
            UpdateEppHelper.P1d_Defect_Epp_Update_For_Realqty(item.Proorder, item.Prodate, item.Prolinename, userid);

            //更新不良集计数据中的实绩生产数量,按工单
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_Realqty(item.Proorder, userid, "ASSYE");

            //判断不良是否录入
            UpdateEppHelper.P1d_Defect_Epp_For_Check(item.Proorder, item.Prodate, item.Prolinename);
        ...
        private DataRow FindRowByID(int rowID)
        {
            ...
            //IQueryable<Pp_P1d_Epp_Outputsub> q = DB.Pp_P1d_Epp_OutputSubs;
            //DataTable OptDatatable =ConvertHelper.IEnumerableConvertToDataTable(q);
            var q = from a in DB.Pp_P1d_Epp_OutputSubs
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete")
            {
                ...
                Pp_P1d_Epp_OutputSub current = DB.Pp_P1d_Epp_OutputSubs.Find(del_ID);
                ...
                //删除记录
                DB.Pp_P1d_Epp_OutputSubs.Where(l => l.ID == del_ID).DeleteFromQuery();
                //重新绑定
                BindGrid();
            }
        }
