...
{
    public partial class p1d_epp_date_daily : PageBase
    {
        #region ViewPower
        ...
        private void LoadData()
        {
            ...
            btnP1dNew.OnClientClick = Window1.GetShowReference("~/Lf_Manufacturing/PP/daily/P1D/p1d_epp_daily_date_new.aspx", "新增") + Window1.GetMaximizeReference();
        ...
        private void BindGrid()
        {
            ...
            IQueryable<Pp_P1d_Epp_Date_Output> q = DB.Pp_P1d_Epp_Date_Outputs; //.Include(u => u.Dept);
            ...
            {
                q = q.Where(u => u.ProStartdate.CompareTo(sdate) >= 0);
            }
            if (!string.IsNullOrEmpty(edate))
            {
                q = q.Where(u => u.ProEnddate.CompareTo(edate) <= 0);
            }
            ...
            //查询包含子集
            var q_include = q.AsEnumerable().Where(p => LineType.Any(g => p.Prolinename == g.DictLabel)).AsQueryable().OrderByDescending(u => u.ProStartdate);
            ...
            // 排列和数据库分页
            q = SortAndPage<Pp_P1d_Epp_Date_Output>(q_include, Grid1);
        ...
        public void BindDdlLine()
        {
            ...
            string sdate = DpStartDate.SelectedDate.Value.ToString("yyyyMMdd");
            string edate = DpEndDate.SelectedDate.Value.ToString("yyyyMMdd");
            var q = from a in DB.Pp_P1d_Epp_Date_Outputs
                        //join b in DB.Pp_EcnSubs on a.Porderhbn equals b.Proecnbomitem
                    where a.ProStartdate.CompareTo(sdate) >= 0
                    where a.ProEnddate.CompareTo(edate) <= 0
                    select new
                    {
                        a.Prolinename
                    };
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            {
                //labResult.Text = keys[0].ToString();
                PageContext.RegisterStartupScript(Window1.GetShowReference("~/Lf_Manufacturing/PP/daily/P1D/p1d_epp_daily_date_edit.aspx?ID=" + keys[0].ToString() + "&type=1") + Window1.GetMaximizeReference());
            }
            if (e.CommandName == "EditOphsub")
            {
                //labResult.Text = keys[0].ToString();
                PageContext.RegisterStartupScript(Window1.GetShowReference("~/Lf_Manufacturing/PP/daily/P1D/p1d_epp_daily_date_sub_edit.aspx?Transtr=" + tracestr + "&type=1") + Window1.GetMaximizeReference());
            }
            ...
            if (e.CommandName == "Delete")
            {
                ...
                Pp_P1d_Epp_Date_Output current = DB.Pp_P1d_Epp_Date_Outputs.Find(del_ID);
                ...
                DB.Pp_P1d_Epp_Date_OutputSubs.Where(l => l.Parent == del_ID).DeleteFromQuery();
                DB.Pp_P1d_Epp_Date_Outputs.Where(l => l.ID == del_ID).DeleteFromQuery();
        ...
        {            // 在操作之前进行权限检查
            ...
            var q =
                ...
                join b in DB.Pp_P1d_Epp_Date_Outputs on p.GUID equals b.GUID
                where p.IsDeleted == 0
                //where p.Prorealtime != 0 || p.Prolinestopmin != 0
                group p by new { b.Prodirect, b.Prostdcapacity, p.Prorealqty, b.Promodel, b.Prohbn, b.Prolot, b.ProStartdate, b.ProEnddate, p.Prostime, p.Proetime, b.Prolinename, p.Prorealtime, p.Prostopcou, p.Prostopmemo, p.Probadcou, p.Probadmemo, p.Prolinemin, p.Prolinestopmin }
                into g
                select new
                {
                    g.Key.ProStartdate,
            ...
            var qs = from g in q
                     select new
                     {
                         生产日期 = g.ProStartdate + "-" + g.ProEnddate,
