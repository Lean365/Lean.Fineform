...
{
    public partial class p1d_epp_daily_date_sub_edit : PageBase
    {
        #region ViewPower
        ...
        private void LoadData()
        {
            ...
            BindDdlStop();
            Rate();
            MemoText.Text = String.Format("<div style=\"margin-bottom:10px;color: #0000FF;\"><strong>填写说明：</strong></div><div>1.EPP生产工数按1天11小时，参与OPH计划台数统计。</div><div>2.同一时段生产多个工单时，计划台数系统自动处理，作业工数计算方法：<strong>多个工单具体的实绩工数+停线时间</strong></div><div>3.实绩工数的计算公式：<strong>作业工数-停线时间</strong></div><div>4.实绩工数不能为负数</div><div>4.生产实绩不能超过工单数量</div>");
        ...
        private void BindGrid()
        {
            ...
            IQueryable<Pp_P1d_Epp_Date_OutputSub> q = DB.Pp_P1d_Epp_Date_OutputSubs; //.Include(u => u.Dept);
            ...
            // 排列和数据库分页
            q = SortAndPage<Pp_P1d_Epp_Date_OutputSub>(q, Grid1);
        ...
        {
            Pp_P1d_Epp_Date_OutputSub item = DB.Pp_P1d_Epp_Date_OutputSubs

                .Where(u => u.ID == rowID).FirstOrDefault();

            Pp_P1d_Epp_Date_Output main_item = DB.Pp_P1d_Epp_Date_Outputs
            ...
            if (rowDict.ContainsKey("Prorealqty"))
            {
                ...
                item.Prorealqty = decimal.Parse(rowData["Prorealqty"].ToString());
            ...
            item.Prostdcapacity = item.Proorderqty;
            if (item.Prolinemin == 0)
            {
                // 完全按照您的代码模式
                DateTime prostime = DateTime.ParseExact(item.Proetime, "yyyyMMdd", CultureInfo.InvariantCulture);
                DateTime proetime = DateTime.ParseExact(item.Prostime, "yyyyMMdd", CultureInfo.InvariantCulture);

                TimeSpan realTime = proetime.Subtract(prostime).Duration();
                int days = realTime.Days + 1;

                item.Prorealtime = (days * 11 * 60) * di - item.Prolinestopmin;
            ...
            //同一小时生产多个工单
            var pcount = from a in DB.Pp_P1d_Epp_Date_OutputSubs
                             //join b in DB.Pp_P1d_Epp_Date_OutputSubs on a.ID equals b.Parent.ID
                         where a.Prostime == item.Prostime
                         where a.ProStartdate == item.ProStartdate
                         where a.ProEnddate == item.ProEnddate
            ...
            if (pcount.Any())
            {
                if (pcount.Count() > 1)
                {
                    ...
                    DB.Pp_P1d_Epp_Date_OutputSubs
                       .Where(s => s.Prostime == item.Prostime)
                       .Where(s => s.ProStartdate == item.ProStartdate)
                       .Where(s => s.ProEnddate == item.ProEnddate)
            ...
            UpdateEppHelper.P1d_Defect_Epp_Update_For_Realqty(item.Proorder, item.ProEnddate, item.Prolinename, userid);

            //更新不良集计数据中的实绩生产数量,按工单
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_Realqty(item.Proorder, userid, "ASSYE");

            //判断不良是否录入
            UpdateEppHelper.P1d_Defect_Epp_For_Check(item.Proorder, item.ProEnddate, item.Prolinename);

            //更新无不良台数
            UpdateEppHelper.P1d_Defect_Epp_Orders_Update_For_NoBadQty(item.Proorder, userid, "ASSYE");
            //更新订单已生产数量
            UpdateEppHelper.UpdateOrderRealQty(item.Proorder, userid);
        }
        ...
        private DataRow FindRowByID(int rowID)
        {
            ...
            //IQueryable<Pp_P1d_Epp_Date_Outputsub> q = DB.Pp_P1d_Epp_Date_OutputSubs;
            //DataTable OptDatatable =ConvertHelper.IEnumerableConvertToDataTable(q);
            var q = from a in DB.Pp_P1d_Epp_Date_OutputSubs
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete")
            {
                ...
                Pp_P1d_Epp_Date_OutputSub current = DB.Pp_P1d_Epp_Date_OutputSubs.Find(del_ID);
                ...
                //删除记录
                DB.Pp_P1d_Epp_Date_OutputSubs.Where(l => l.ID == del_ID).DeleteFromQuery();
                //重新绑定
                BindGrid();
            }
        }
