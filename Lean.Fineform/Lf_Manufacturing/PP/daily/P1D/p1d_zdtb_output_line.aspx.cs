...
{
    public partial class p1d_zdtb_output_line : PageBase
    {
        #region ViewPower
        ...
        //    Prefix_XlsxName = DpStartDate.SelectedDate.Value.ToString("yyyyMM") + "_Output(Line)_Report";
        ...
        protected void BtnRepair_Click(object sender, EventArgs e)
        {
            ...
            // mysql = "SELECT [Prodate] 日付,[Prohbn] 品目,[Prost] ST,[Proplanqty] 計画台数,[Proworktime] 投入工数,[Proworkqty] 実績台数,[Prodirect] 直接人数,[Proworkst] 実績ST,[Prodiffst] ST差異,[Prodiffqty] 台数差異,[Proactivratio] 稼働率  FROM [dbo].[Pp_Outputlinedatas] where left(Prodate,6)='" + DDLdate.SelectedText + "'";
            Prefix_XlsxName = DpStartDate.SelectedDate.Value.ToString("yyyyMM") + "_Rework(Line)_Output_Report";
            //mysql = "EXEC DTA.dbo.SP_BOM_EXPAND '" + Prefix_XlsxName + "'";
            Export_FileName = Prefix_XlsxName + ".xlsx";
            ...
            if (q.Any())
            {
                ...
                ExportHelper.ReworkLine_XlsxFile(ConvertHelper.LinqConvertToDataTable(qss), "R" + DpStartDate.SelectedDate.Value.ToString("yyyyMM"), Export_FileName, DpEndDate.SelectedDate.Value.ToString("yyyyMM"));
